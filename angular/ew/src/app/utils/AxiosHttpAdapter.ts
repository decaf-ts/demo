...
import { HttpAdapter, HttpConfig } from "@decaf-ts/for-http";
import { Context, RepositoryFlags } from "@decaf-ts/db-decorators";

interface AxiosFlags extends RepositoryFlags {
...
> {
   ...
   * @param {Record<string, unknown>} model - The data model to create
   * @return {Promise<Record<string, unknown>>} A promise that resolves with the created resource
   */
  async create(
    tableName: string,
    id: string | number,
   ...
   * @return {Promise<Record<string, unknown>>} A promise that resolves with the retrieved resource
   */
  async read(
    tableName: string,
   ...
   * @param {Record<string, unknown>} model - The updated data model
   * @return {Promise<Record<string, unknown>>} A promise that resolves with the updated resource
   */
  async update(
    tableName: string,
    id: string | number,
   ...
   * @return {Promise<Record<string, unknown>>} A promise that resolves with the deletion result
   */
  async delete(
    tableName: string,
    id: string | number | bigint
