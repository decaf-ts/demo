...
import { getLogger } from "@decaf-ts/for-angular";
...
export class NgxLoadingComponent {
  ...
  async show(message: string, options?: LoadingOptions): Promise<void> {
    try {
      ...
      getLogger(this).error(error as ErrorLike);
    }
  }
  ...
  async remove(): Promise<void> {
    ...
    if (!loading)
      return getLogger(this).info("Try Remove loading but no loading is present. Promise resolve will be called anyway");
